    ...
    if (AVObject.class.isAssignableFrom((Class) type)) {
      ...
      try {
        ...
        if (objectMap.containsKey("keyValues")) {
          ...
          Map<String, JSONObject> keyValues =
              (Map<String, JSONObject>) objectMap.get("keyValues");
          for (Map.Entry<String, JSONObject> entry : keyValues.entrySet()) {
            JSONObject keyValue = entry.getValue();
          ...
          if (objectMap.containsKey("operationQueue")) {
            object.operationQueue.putAll((Map<String, AVOp>) objectMap
                .get("operationQueue"));
          }
